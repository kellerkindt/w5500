...
/// Fixed-length Data Mode for communicating with the chip.  Requires three connection wires:
///
/// - MISO
/// - MOSI
/// - CLK
pub struct Fdm<Spi: Transfer<u8> + Write<u8>> {
    spi: Spi,
}

impl<Spi: Transfer<u8> + Write<u8>> Fdm<Spi> {
...
impl<Spi: Transfer<u8> + Write<u8>> Bus for Fdm<Spi> {
    type Error = FdmError<<Spi as Transfer<u8>>::Error, <Spi as Write<u8>>::Error>;
    ...
    fn read_frame(
        ...
        while !data_phase.is_empty() {
            ...
            self.spi
                ...
                .map_err(FdmError::WriteError)?;
            self.spi
                .transfer(&mut data_phase[..last_length_written as usize])
                .map_err(FdmError::TransferError)?;
...
pub enum FdmError<TransferError, WriteError> {
    TransferError(TransferError),
    WriteError(WriteError),
}

impl<TransferError, WriteError> fmt::Debug for FdmError<TransferError, WriteError> {
